...
@objc public class TSMotionPermissionManager: NSObject {

    private static var _sharedInstance: TSMotionPermissionManager?
    private static let lock = NSLock()

    private let motionManager = CMMotionActivityManager()

    @objc public class func sharedInstance() -> TSMotionPermissionManager {
        lock.lock()
        defer { lock.unlock() }
        if _sharedInstance == nil {
            _sharedInstance = TSMotionPermissionManager()
        }
