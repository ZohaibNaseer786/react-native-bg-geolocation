...
@objc public final class TSActivityConfig: TSConfigModuleBase {
    ...
    @objc public override func propertySpecs() -> [TSPropertySpecImpl] {
        return [
            TSPropertySpecImpl(name: "stopDetectionDelay", type: "integer"),
            TSPropertySpecImpl(name: "activityRecognitionInterval", type: "integer"),
            TSPropertySpecImpl(name: "minimumActivityRecognitionConfidence", type: "integer"),
            TSPropertySpecImpl(name: "disableMotionActivityUpdates", type: "bool"),
            TSPropertySpecImpl(name: "disableStopDetection", type: "bool"),
            TSPropertySpecImpl(name: "stopOnStationary", type: "bool"),
            TSPropertySpecImpl(name: "triggerActivities", type: "string")
        ]
    }
    ...
    @objc public override func validateConfiguration() -> Bool {
        if disableStopDetection && stopOnStationary {
            ...
            if logger.shouldLog(2) {
                let message = "TSActivityConfig: stopOnStationary is enabled but disableStopDetection is also enabled - stopOnStationary will have no effect"
                logger.log(2, tag: 9, function: "-[TSActivityConfig validateConfiguration]", message: message)
            }
        }
        if disableMotionActivityUpdates && !disableStopDetection {
            let logger = TSLog.sharedInstance()
            if logger.shouldLog(2) {
                let message = "TSActivityConfig: Motion activity updates are disabled but stop detection is enabled - stop detection may not work properly"
                logger.log(2, tag: 9, function: "-[TSActivityConfig validateConfiguration]", message: message)
            }
        }
        return true
