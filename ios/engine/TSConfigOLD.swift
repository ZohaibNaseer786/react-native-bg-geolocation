...
@objc public class TSConfigBuilder: NSObject {
    ...
    @objc public class func decodeActivityType(_ type: String) -> CLActivityType {
        return TSGeolocationConfig.activityType(fromString: type) ?? .other
    }

    @objc public class func decodeDesiredAccuracy(_ accuracy: Any) -> CLLocationAccuracy {
        return TSGeolocationConfig.decodeDesiredAccuracy(accuracy)
    }
...
@objc public class TSConfigOLD: NSObject, NSCoding {

    private static var _sharedInstance: TSConfigOLD?
    private static let instanceLock = NSLock()
    private static let storageKey = "TSLocationManager_config_v2"

    @objc public class func sharedInstance() -> TSConfigOLD {
        instanceLock.lock()
        defer { instanceLock.unlock() }
        if _sharedInstance == nil {
            _sharedInstance = TSConfigOLD()
        }
        return _sharedInstance!
    }

    @objc public class func decodeConfig(_ dict: [String: Any]) -> TSConfigOLD {
        let config = TSConfigOLD()
    ...
    @objc public func updateWithBlock(_ block: (TSConfigBuilder) -> Void) {
        let builder = TSConfigBuilder()
        block(builder)
    ...
    @objc public func persist() {
        ...
        UserDefaults.standard.set(data, forKey: TSConfigOLD.storageKey)
