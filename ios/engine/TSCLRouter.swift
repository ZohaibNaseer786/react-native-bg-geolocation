...
/// `TSTrackingService`, `TSGeofenceManager`, `TSLocationRequestService` and
/// `TSLocationAuthorization` each assigned `manager.delegate = self`, so whoever
...
/// the auth callbacks never reached `TSLocationAuthorization`, so
...
@objc public class TSCLRouter: NSObject, CLLocationManagerDelegate {

    private static var _sharedInstance: TSCLRouter?
    private static let instanceLock = NSLock()

    @objc public class func sharedInstance() -> TSCLRouter {
        instanceLock.lock()
        defer { instanceLock.unlock() }
        if _sharedInstance == nil { _sharedInstance = TSCLRouter() }
        return _sharedInstance!
    }

    private var tracking: TSTrackingService { TSTrackingService.sharedInstance() }
    private var geofences: TSGeofenceManager { TSGeofenceManager.sharedInstance() }
    private var requests: TSLocationRequestService { TSLocationRequestService.sharedInstance() }
    private var authorization: TSLocationAuthorization { TSLocationAuthorization.sharedInstance() }
    ...
    @objc public func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // fix to TSGeofenceManager.setLocation internally. Only when tracking is
    ...
    @objc public func locationManager(_ manager: CLLocationManager, didExitRegion region: CLRegion) {
        // "TSStationary" -> tracking (start-detection); "TSGeofence:" -> geofences.
