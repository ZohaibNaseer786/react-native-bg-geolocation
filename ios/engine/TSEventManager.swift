...
@objc public class TSLocationListener: NSObject {
    @objc public var success: ((Any?) -> Void)?
    @objc public var failure: ((Any?) -> Void)?

    @objc public override func isEqual(_ object: Any?) -> Bool {
        guard let other = object as? TSLocationListener else { return false }
        return self === other
    }
...
@objc public class TSEventManager: NSObject {

    private static var _sharedInstance: TSEventManager?
    ...
    @objc public class func sharedInstance() -> TSEventManager {
        lock.lock()
        defer { lock.unlock() }
        if _sharedInstance == nil {
            _sharedInstance = TSEventManager()
        }
    ...
    @objc public func addLocationListener(success: @escaping (Any?) -> Void,
                                           failure: @escaping (Any?) -> Void) -> TSLocationListener {
        let listener = TSLocationListener()
    ...
    @objc public func triggerLocationSuccess(_ location: Any?) {
        ...
        objc_sync_exit(self)
        for listener in lst {
            (listener as? TSLocationListener)?.success?(location)
        }
    }
    ...
    @objc public func triggerLocationError(_ error: Any?) {
        ...
        objc_sync_exit(self)
        for listener in lst {
            (listener as? TSLocationListener)?.failure?(error)
        }
    }
