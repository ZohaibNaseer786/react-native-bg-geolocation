...
@objc public class TSDataStore: NSObject {

    private static var _sharedInstance: TSDataStore?
    ...
    @objc public class func sharedInstance() -> TSDataStore {
        lock.lock()
        defer { lock.unlock() }
        if _sharedInstance == nil {
            _sharedInstance = TSDataStore()
        }
