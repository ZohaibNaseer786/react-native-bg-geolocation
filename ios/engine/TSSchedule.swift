...
@objc public final class TSSchedule: NSObject {
    ...
    @objc public func trigger(_ state: Bool) {
        ...
        if state {
            if logger.shouldLog(5) {
                ...
                logger.log(5, tag: 3, function: "-[TSSchedule trigger:]", message: message)
            }
        } else {
            if logger.shouldLog(3) {
                let message = String(format: "%@", description)
                logger.log(3, tag: 5, function: "-[TSSchedule trigger:]", message: message)
            }
        }
        hasTriggered = true
        triggered = state
        ...
        let event = TSScheduleEvent(schedule: self, state: state)
        TSEventBus.sharedInstance().trigger(TSEventNameSchedule, payload: event)
        TSEventManager.sharedInstance().trigger(TSEventNameSchedule, payload: event)
    }
    ...
    public override var description: String {
        ...
        return String(format: "TSSchedule[triggered:%d, %02ld:%02ld - %02ld:%02ld, Days:%@, trackingMode:%d]",
