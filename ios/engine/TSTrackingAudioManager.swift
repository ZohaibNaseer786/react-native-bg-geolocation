...
@objc public final class TSTrackingAudioManager: NSObject {
    @objc public static let shared = TSTrackingAudioManager()

    private let audioQueue = DispatchQueue(label: "TSTrackingAudioManager.audio")
    ...
    @objc public func startIfNeeded() {
        guard TSConfig.sharedInstance().app.trackingAudioEnabled else { return }
        audioQueue.sync {
    ...
    @objc public func stateDictionary() -> [String: Any] {
        audioQueue.sync {
            var state: [String: Any] = [
                "enabled": TSConfig.sharedInstance().app.trackingAudioEnabled,
                "requested": shouldBeRunning,
                "active": active,
                "audible": true,
                "volume": TSConfig.sharedInstance().app.trackingAudioVolume,
    ...
    @objc private func applicationDidFinishLaunching() {
        audioQueue.async {
            if self.shouldBeRunning || TSConfig.sharedInstance().enabled {
                self.startLocked()
            }
        }
    }
    ...
    private func startLocked() {
        guard shouldBeRunning || TSConfig.sharedInstance().enabled else { return }
        guard TSConfig.sharedInstance().app.trackingAudioEnabled else { return }
    ...
    private func publishNowPlaying() {
        let app = TSConfig.sharedInstance().app
        let appName =
